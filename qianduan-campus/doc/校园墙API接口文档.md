...
public class PostItemVO {
    ...
    private String authorAvatar; // 匿名则为null，实名时返回用户头像URL
    private Integer likeCount;
    private Integer commentCount;
    private String createTime;
}
...
public class CommentItemVO {
    ...
    private String authorAvatar; // 评论作者头像URL
    private String createTime;
}
```
...
- **接口说明**：上传当前用户头像图片，返回可直接保存到 `avatar` 字段的图片 URL
- **请求方式**：POST
...
| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| file | file | 是 | 头像图片文件，建议 jpg/png/webp，最大 5MB |
...
```json
{
    "code": 200,
    "message": "上传成功",
    "data": "https://xxx.com/avatar/user_1_1710000000.jpg"
}
```
...
{
    "nickname": "小王",
    "avatar": "https://xxx.com/avatar/user_1_1710000000.jpg"
}
```
        ...
        "comments": [
            {
                ...
                "content": "确实，我也觉得贵了",
                "authorName": "李四",
                "authorAvatar": "https://xxx.com/avatar/2.jpg",
...
- `authorAvatar` 用于前端显示帖子作者头像
- `CommentItemVO.authorAvatar` 用于前端显示评论作者头像
- 当帖子为匿名发布时：
  - `authorName` 返回 `"匿名用户"`
  - `authorAvatar` 返回 `null`
- 当评论需要匿名能力时，也建议沿用相同规则；如果评论当前不支持匿名，则直接返回评论作者头像

### 3.3.2 后端实现建议

为了支持帖子和评论头像展示，建议后端按下面方式调整：

#### 需要补充字段的类

1. `PostItemVO`
   - 新增 `private String authorAvatar;`
2. `PostDetailVO`
   - 新增 `private String authorAvatar;`
3. `CommentItemVO`
   - 新增 `private String authorAvatar;`
...
- `User` 实体已经有 `avatar` 字段，无需新增
- `Post` 实体不需要新增头像字段，继续通过 `userId` 关联用户表即可
- `Comment` 实体同样不需要新增头像字段，继续通过评论用户ID关联用户表即可

#### 查询层建议

帖子列表、帖子详情、评论列表在查询时都应关联 `user.avatar`：

1. 帖子列表查询
   - 在原有帖子分页查询基础上，关联发帖用户表
   - 取出：
     - `user.nickname`
     - `user.username`
     - `user.avatar`
...
在 Service 层组装 `PostItemVO / PostDetailVO / CommentItemVO` 时建议统一处理：

1. 帖子匿名时
   - `authorName = "匿名用户"`
   - `authorAvatar = null`
2. 帖子实名时
   - `authorName` 优先取 `nickname`，没有则取 `username`
...
建议把“用户名/头像显示规则”抽成统一的转换方法，例如：

```java
private void fillPostAuthorInfo(Post post, User user, PostItemVO vo) {
    if (post.getIsAnonymous() != null && post.getIsAnonymous() == 1) {
        vo.setAuthorName("匿名用户");
        vo.setAuthorAvatar(null);
    } else {
        vo.setAuthorName(StrUtil.isNotBlank(user.getNickname()) ? user.getNickname() : user.getUsername());
        vo.setAuthorAvatar(user.getAvatar());
    }
}
```

这样可以避免帖子列表、帖子详情、评论列表三处重复写判断逻辑，后续也更容易统一维护。
...
| 用户 | 上传用户头像 | POST | /api/user/avatar | 是 |
| 用户 | 修改用户信息 | PUT | /api/user/update | 是 |
| 用户 | 修改密码 | PUT | /api/user/password | 是 |
| 首页 | 获取首页统计数据 | GET | /api/home/stats | 否 |
